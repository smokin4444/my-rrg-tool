...
# --- NEW ROBUST DATA ENGINE ---
@st.cache_data(ttl=600)
def download_data(tickers, interval):
    period = "2y"
    try:
        # Optimization: Fetch all at once, auto_adjust=True to prevent Adj Close mismatch
        data = yf.download(tickers, period=period, interval=interval, progress=False, auto_adjust=True)
        if data.empty:
            st.error("Engine failed to receive data from Yahoo Finance. API might be temporarily down.")
            return pd.DataFrame()
        return data
...
def run_dual_analysis(ticker_str, bench, tf_display):
    ...
    for t in tickers:
        ...
        if res_display is not None and not res_display.empty:
            ...
            table_data.append({"Ticker": t, "Full Name": TICKER_NAMES.get(t, t), "Sync Status": sync, "Daily Stage": stg_d, "Weekly Stage": stg_w, "Rotation Score": round((res_display['x'].iloc[-1] * 0.5) + (res_display['y'].iloc[-1] * 0.2), 1)})
...
try:
    ...
    if not df_main.empty:
        ...
        fig.add_vrect(x0=POWER_WALK_LEVEL, x1=CHART_RANGE[1], fillcolor="#2ECC71", opacity=0.1, layer="below", line_width=0, annotation_text="POWER WALK ZONE", annotation_position="top left", annotation_font=dict(color="#27ae60", size=10))
