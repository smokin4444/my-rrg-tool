...
# --- TICKER DICTIONARY (Restored & Expanded) ---
TICKER_NAMES = {
...
with st.sidebar:
    ...
    # Auto-benchmark: Absolute (ONE) for Assets/Income, SPY for others
    ...
    if bench_data is not None:
        ...
        for i, t in enumerate(active_tickers):
            ...
            if p_display is not None:
                ...
                # Calculate Daily/Weekly for Sync Table
                p_d = fetch_safe(t, "Daily")
                b_d = pd.Series(1.0, index=pd.date_range(end=pd.Timestamp.now(), periods=500)) if is_abs else fetch_safe(benchmark, "Daily")
                m_daily = calculate_rrg(p_d, b_d, t)
                
                p_w = fetch_safe(t, "Weekly")
                b_w = pd.Series(1.0, index=pd.date_range(end=pd.Timestamp.now(), periods=500)) if is_abs else fetch_safe(benchmark, "Weekly")
                m_weekly = calculate_rrg(p_w, b_w, t)
        ...
        if results_map:
            ...
            for t, data in results_map.items():
                ...
                table_rows.append({
                    ...
                    "Sync Status": sync, "Daily": q_d, "Weekly": q_w, 
                    "Score": round(data['chart']['x'].iloc[-1], 2)
                })
            
            final_df = pd.DataFrame(table_rows).sort_values("Score", ascending=False)
            st.dataframe(final_df.style.applymap(lambda v: "background-color: #2ecc71; color: white" if v == "💎 BULLISH SYNC" else "background-color: #3498db; color: white" if v == "📈 PULLBACK BUY" else "", subset=["Sync Status"]), use_container_width=True)
        else:
            st.warning("Data fetch returned empty. Check ticker symbols.")
