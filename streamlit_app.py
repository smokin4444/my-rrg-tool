...
def download_data(tickers, interval):
    # Batch download to prevent throttling
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        try:
            # Requesting JUST the 'Close' to keep payload light
            data = yf.download(chunk, period=period, interval=interval, progress=False)
            if data is not None and not data.empty:
                # Handle YFinance multi-index vs single ticker return
...
def run_dual_analysis(ticker_str, bench, tf_display):
    ...
    # Only request the specific timeframe needed to reduce 'NoneType' errors
    interval = "1d" if tf_display == "Daily" else "1wk"
    data_all = download_data(list(set(tickers + ([bench_t] if not is_absolute else []))), interval)
    
    if data_all is None:
        st.warning("⚠️ Data currently unavailable from Yahoo Finance. Try a different timeframe or Reset Engine.")
        return pd.DataFrame(), {}
    
    hist_out, table_data = {}, []
    for t in tickers:
        res = get_metrics(data_all, t, bench_t, is_absolute)
        if res is not None:
            hist_out[t] = res
            x, y = res['x'].iloc[-1], res['y'].iloc[-1]
            stg = "LEADING" if x >= 100 and y >= 100 else "LAGGING"
