...
# Added ASX List
ASX_BLUE_CHIPS = "CBA.AX, BHP.AX, CSL.AX, NAB.AX, WBC.AX, ANZ.AX, MQG.AX, WES.AX, TLS.AX, WOW.AX, FMG.AX, RIO.AX, GMG.AX, TCL.AX, ALL.AX"
MINERS = "AFM.V, NAK, A4N.AX, CSC.AX, IVN.TO, TGB, MU, APLD"

FUND_MAP = {"CBA.AX": "CommBank", "BHP.AX": "BHP Group", "STW.AX": "ASX 200 ETF", "VAS.AX": "Vanguard ASX"}
...
with st.sidebar:
    ...
    if heap_type == "Master Themes": current_list, def_bench = MASTER_THEMES, "SPY"
    elif heap_type == "Startup": current_list, def_bench = STARTUP_THEMES, "SPY"
    elif heap_type == "ASX Blue Chips": current_list, def_bench = ASX_BLUE_CHIPS, "STW.AX"
    elif heap_type == "My Miners": current_list, def_bench = MINERS, "SPY"
    else: current_list, def_bench = st.session_state.get('custom_list', MASTER_THEMES), "SPY"

    tickers_input = st.text_area("Ticker Heap:", value=current_list, height=200).replace("HG1!", "HG=F")
    benchmark = st.text_input("Benchmark:", value=def_bench)
    st.markdown("---")
...
def run_analysis(ticker_str, bench):
    ...
    # Download with longer period for ASX liquidity
    ...
    for t in tickers:
        ...
        if d_res and w_res:
            ...
            table_data.append({"Ticker": t, "Name": FUND_MAP.get(t, ""), "Sync Status": status, "Daily Quad": dq, "Weekly Quad": wq, "Daily CH": d_res[1], "RS-Ratio": round(dr, 2), "Rel Vol": d_res[2]})
...
try:
    ...
    for i, (t, df) in enumerate(history_data[timeframe].items()):
        ...
        if tail_len > 1:
            fig.add_trace(go.Scatter(x=df_p['x'], y=df_p['y'], mode='lines+markers', name=t, line=dict(color=color, width=2, shape='spline'), marker=dict(size=8, opacity=0.7), customdata=np.stack((df_p['date_label'], df_p['quad']), axis=-1), hovertemplate="<b>" + t + "</b><br>Date: %{customdata[0]}<br>Quad: %{customdata[1]}<extra></extra>", opacity=0.4, legendgroup=t))
    ...
    st.error(f"Error: {e}. Check if Ticker and Benchmark are from the same exchange.")
