...
FUND_MAP = {"SOXX": "Semis", "IBIT": "Bitcoin", "COPX": "Copper", "XLRE": "Real Estate", "ASTS": "AST SpaceMobile"}
...
def get_rrg_metrics(df_raw, ticker, b_ticker):
    try:
        ...
        return pd.DataFrame({'x': ratio, 'y': mom, 'date': ratio.index}).dropna(), round(ch, 2), rv
    except: return None
...
def run_analysis(ticker_str, bench):
    ...
    data = yf.download(list(set(tickers + [bench.upper()])), period="2y", interval="1d", group_by='ticker', progress=False)
    w_data = yf.download(list(set(tickers + [bench.upper()])), period="2y", interval="1wk", group_by='ticker', progress=False)
    
    history, table_data = {"Daily": {}, "Weekly": {}}, []
    for t in tickers:
        d_res = get_rrg_metrics(data, t, bench.upper())
        w_res = get_rrg_metrics(w_data, t, bench.upper())
        if d_res and w_res:
