...
TICKER_NAMES = {
    ...
    "BDRY": "Dry Bulk Shipping", "MOO": "Agribusiness",
...
# --- THEME TRACKER TABLE ---
def display_deepvue_theme_tracker(history_dict):
    st.markdown("---")
    st.subheader("🔥 DeepVue Theme Tracker (Heat Score)")
    theme_data = []
    total_power = 0
    for ticker, data in history_dict.items():
        curr_x = data['x'].iloc[-1]
        prev_x_1w = data['x'].iloc[-2] if len(data) >= 2 else curr_x
        chg_1w = curr_x - prev_x_1w
        is_power = 1 if curr_x >= POWER_WALK_LEVEL else 0
        total_power += is_power
        theme_data.append({
            "Theme/Ticker": ticker,
            "Full Name": TICKER_NAMES.get(ticker, ticker),
            "Current RS": round(curr_x, 2),
            "1W Δ RS": round(chg_1w, 2),
            "Status": "🔥 ACCELERATING" if chg_1w > 0 else "🧊 COOLING"
        })
    
    heat_score = (total_power / len(history_dict)) * 100 if history_dict else 0
    st.metric("Group Heat Score", f"{int(heat_score)}%", help="Percentage of group in Power Walk Zone")
    
    tracker_df = pd.DataFrame(theme_data).sort_values("1W Δ RS", ascending=False)
    def style_tracker(val):
        if isinstance(val, float): return f'color: {"#2ecc71" if val > 0 else "#e74c3c"}'
        if val == "🔥 ACCELERATING": return 'color: #2ecc71; font-weight: bold'
        if val == "🧊 COOLING": return 'color: #e74c3c'
        return ''
    st.dataframe(tracker_df.style.applymap(style_tracker, subset=["1W Δ RS", "Status"]), use_container_width=True)
...
try:
    ...
    if not df_main.empty:
        ...
        fig.add_vrect(x0=POWER_WALK_LEVEL, x1=CHART_RANGE[1], fillcolor="#2ECC71", opacity=0.1, layer="below", line_width=0, annotation_text="POWER WALK", annotation_position="top left")
        ...
        # New Theme Tracker Call
        display_deepvue_theme_tracker(hist)
